...
df <-readRDS(paste0('data/preprocessed/df_', quarter, year, 'only_ground_truth_tags', '.rds'))
...
ground_truth <- readr::read_csv('data/raw/ground_truth.csv') %>%
  ...
  select(cat, object) %>%
  unique() %>% 
  mutate(cat = case_when(str_detect(cat, 'sql') ~ 'sql',
...
F1_h <- function(quarter, year, only_ground_truth_tags, i) {
  ...
  return(2 * ((F_XY(communities, ground_truth) * F_XY(ground_truth, communities)) / 
                (F_XY(communities, ground_truth) + F_XY(ground_truth, communities))))
}

F1_h_df <- tibble(i = seq(1000))
F1_h_list = c()
...
for(i in seq(1000)) {
  ...
  F1_h_val <- F1_h(quarter, year, 'only_ground_truth_tags', i)
...
t.test(F1_h_df$F1_h_vals, alternative = "less", mu = 1, conf.level = 0.95)
